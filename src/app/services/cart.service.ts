...
imagenes: any
  ...
  CalculateSubTotal(index): Number {
    ...
    // @ts-ignore
    subTotal = p.product.precio * p.numInCart;
  ...
  UpdateCartData(index, increase: Boolean) {
    ...
    if (increase) {
      // @ts-ignore
      data.numInCart < data.product.cantidad ? data.numInCart++ : data.product.cantidad;
    ...
    this.httpClient.post(`${this.ServerURL}orders/payment`, null).subscribe((res: { success: Boolean }) => {
     console.clear();
      ...
      if (res.success) {
        ...
        this.httpClient.post(`${this.ServerURL}orders/new`, {
          ...
          this.orderService.getSingleOrder(data.order_id).then(prods => {
            if (data.success) {
              ...
              this.spinner.hide();
              this.router.navigate(['/terminado'], navigationExtras).then(p => {
                this.cartDataClient = {prodData: [{incart: 0, id: 0}], total: 0};
        ...
        this.spinner.hide();
        this.router.navigateByUrl('/revisado').then();
